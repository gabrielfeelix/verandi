...
export function AceitarConvite({ token, email }: { token: string; email: string }) {
  ...
  return (
    ...
    >
      <Campo rotulo="E-mail" htmlFor="c-email">
        <input id="c-email" className={`${entrada} opacity-70`} value={email} readOnly />
      </Campo>

      <Campo rotulo="Senha" htmlFor="c-senha" dica="ao menos 8 caracteres">
        <input id="c-senha" name="senha" type="password" required minLength={8}
          className={entrada} autoFocus />
      </Campo>

      <Campo rotulo="Repita a senha" htmlFor="c-repete">
        <input id="c-repete" name="repete" type="password" required minLength={8}
          className={entrada} />
      </Campo>

      {erro ? <Nota tom="alerta">{erro}</Nota> : null}

      <Botao type="submit" disabled={pendente}>Entrar na conta</Botao>
    </form>
  )
}
